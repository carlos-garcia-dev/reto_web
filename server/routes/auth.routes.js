...
router.post('/signup', (req, res) => {
    ...
    if (!name || !username || !email || !password) {
        res.status(400).json({ message: 'Fill the password and fields' })
        return
    }
    ...
    User
        ...
        .then(user => {
            ...
            User 
                .create({ name, email, name, password: hashPass })
...
router.post('/signin', (req, res) => {
    ...
    passport.authenticate('local', (err, user, failure) => {
        ...
        if (err) {
            res.status(500).json({ message: 'It was not possible to authenticate' })
            return
        }
    
        if (!user) {
            req.status(401).json(failure)
        }
